#include "TransformPlugin002.h"

#include <opencv2/imgproc/types_c.h>

#include "ui_plugin.h"

openCVInterface::TransformPlugin002::TransformPlugin002( ) {
...
QString openCVInterface::TransformPlugin002::title( ) {
	return this->metaObject()->className();
}

QString openCVInterface::TransformPlugin002::version( ) {
	return "1.0.0";
}

QString openCVInterface::TransformPlugin002::description( ) {
	return "Performs different transformations available in OpenCV";
}

QString openCVInterface::TransformPlugin002::help( ) {
	return "This is a plugin that performs different filters available in OpenCV";
}

void openCVInterface::TransformPlugin002::setupUi( QWidget *parent ) {
	ui = new Ui::PluginGui;
	ui->setupUi(parent);
...
void openCVInterface::TransformPlugin002::processImage( const cv::Mat &inputImage, cv::Mat &outputImage ) {
	using namespace cv;
...
void openCVInterface::TransformPlugin002::on_resizeHalfRadio_toggled( bool checked ) {
	Q_UNUSED(checked);
	emit updateNeeded();
}

void openCVInterface::TransformPlugin002::on_resizeDoubleRadio_toggled( bool checked ) {
	Q_UNUSED(checked);
	emit updateNeeded();
}

void openCVInterface::TransformPlugin002::on_remapRadio_toggled( bool checked ) {
	Q_UNUSED(checked);
	emit updateNeeded();
}

void openCVInterface::TransformPlugin002::on_affineRadio_toggled( bool checked ) {
	Q_UNUSED(checked);
	emit updateNeeded();
}

void openCVInterface::TransformPlugin002::on_perspectiveRadio_toggled( bool checked ) {
	Q_UNUSED(checked);
	emit updateNeeded();
}

void openCVInterface::TransformPlugin002::on_borderTypeCombo_currentIndexChanged( int index ) {
	Q_UNUSED(index);
	emit updateNeeded();
}

void openCVInterface::TransformPlugin002::on_interpolationCombo_currentIndexChanged( int index ) {
	Q_UNUSED(index);
	emit updateNeeded();
}