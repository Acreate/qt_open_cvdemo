#include "TransformPlugin001.h"

#include <opencv2/imgproc/types_c.h>

#include "ui_plugin.h"

openCVInterface::TransformPlugin001::TransformPlugin001()
{
...
QString openCVInterface::TransformPlugin001::title()
{
    return this->metaObject()->className();
}

QString openCVInterface::TransformPlugin001::version()
{
    return "1.0.0";
}

QString openCVInterface::TransformPlugin001::description()
{
    return "Performs different transformations available in OpenCV";
}

QString openCVInterface::TransformPlugin001::help()
{
    return "This is a plugin that performs different filters available in OpenCV";
}

void openCVInterface::TransformPlugin001::setupUi(QWidget *parent)
{
    ui = new Ui::PluginGui;
    ui->setupUi(parent);
...
void openCVInterface::TransformPlugin001::processImage(const cv::Mat &inputImage, cv::Mat &outputImage)
{
    using namespace cv;
...
void openCVInterface::TransformPlugin001::on_resizeHalfRadio_toggled(bool checked)
{
    Q_UNUSED(checked);
    emit updateNeeded();
}

void openCVInterface::TransformPlugin001::on_resizeDoubleRadio_toggled(bool checked)
{
    Q_UNUSED(checked);
    emit updateNeeded();
}

void openCVInterface::TransformPlugin001::on_remapRadio_toggled(bool checked)
{
    Q_UNUSED(checked);
    emit updateNeeded();
}

void openCVInterface::TransformPlugin001::on_affineRadio_toggled(bool checked)
{
    Q_UNUSED(checked);
    emit updateNeeded();
}

void openCVInterface::TransformPlugin001::on_perspectiveRadio_toggled(bool checked)
{
    Q_UNUSED(checked);
    emit updateNeeded();
}

void openCVInterface::TransformPlugin001::on_borderTypeCombo_currentIndexChanged(int index)
{
    Q_UNUSED(index);
    emit updateNeeded();
}

void openCVInterface::TransformPlugin001::on_interpolationCombo_currentIndexChanged(int index)
{
    Q_UNUSED(index);
    emit updateNeeded();
}
